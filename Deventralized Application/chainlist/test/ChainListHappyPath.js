...
contract('ChainList', function (accounts) {
   ...
   it("should sell an article", function () {
      return ChainList.deployed().then(function (instance) {
         ...
         assert.equal(data[4], web3.utils.toWei(`${articlePrice}`, "ether"), "article price must be " + web3.utils.toWei(`${articlePrice}`, "ether"));
         console.log("article sold")
      });
   });
   ...
   it("should buy an article", function () {
      ...
      //    sellerBalanceBeforeBuy = web3.utils.fromWei(`${sellerBalance}`, "ether");
      //    buyerBalanceBeforeBuy = web3.utils.fromWei(`${buyerBalance}`, "ether");
      ...
      //    assert.equal(receipt.logs[0].args._price, web3.utils.toWei(`${articlePrice}`, "ether"), "event article price must be " + web3.utils.toWei(`${articlePrice}`, "ether"));

      //    // record balances of buyer and seller after the buy
      //    sellerBalanceAfterBuy = web3.fromWei(web3.eth.getBalance(seller), "ether");
      //    buyerBalanceAfterBuy = web3.fromWei(web3.eth.getBalance(buyer), "ether");
      ...
      //    assert.equal(data[2], articleName, "article name must be " + articleName);
      //    assert.equal(data[3], articleDescription, "article description must be " + articleDescription);
      //    assert.equal(data[4].toNumber(), web3.utils.toWei(`${articlePrice}`, "ether"), "article price must be " + web3.utils.toWei(`${articlePrice}`, "ether"));
      // });
   });
